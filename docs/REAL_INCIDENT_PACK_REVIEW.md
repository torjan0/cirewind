...
| `assemble-candidate` | review-unit root with hand-authored `pack.yaml`, `sources.json`, `claims.json`, `conflicts.json`, and archived source objects; repository root; review policy profile; preparer, author, and transcriber identities | Canonicalizes the hand-authored ledgers, retains the current repository review policy, writes the registered generator's fixture snapshots and index, replays each snapshot through the production derivation into `expected-findings.json` (aborting on any forbidden state), and writes `packet.json`, `validation.json`, and both manifests. Deterministic and idempotent; it records no approval, candidate commit, or factual certification. |
| `extract-indicators` | extractor identifier, one pinned local source file (the GitHub REST advisory object or the matching-refs tag listing), output path, and for the tag inventory the unrestored names the advisory states | Transforms the source bytes into a sealed, typed, sorted extraction record (input byte length and SHA-256, extractor identity and version, every normalization applied, output SHA-256) and writes it as canonical JSON. Rejects altered headings, malformed or duplicate rows, and oversize input rather than skipping them; never uses the network and never guesses a digest namespace. The record is review input that candidate claims cite; it certifies nothing. |
...
The maintainer-side sequence from requesting reviews to promotion, with the
identities of the three v0.2 candidates, is in
[`PACK_APPROVAL_RUNBOOK.md`](PACK_APPROVAL_RUNBOOK.md).
